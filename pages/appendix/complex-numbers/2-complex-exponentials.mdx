...
Wenn man das Zeitintervall also in immer kleinere Intervalle einteilt und somit noch mehr Gebrauch vom Zinseszins macht, kann man dann durch $n \to \infty$ immer mehr Geld bekommen?
...
Natürlich sind komplexe Zahlen als Zinsen auch unrealisitsch. Es ist jedoch bekannt, wie man komplexe Zahlen dividiert, addiert und multipliziert und somit den Term $\lim_{n\to \infty} \left(1+\frac{x}{n}\right)^{n}$ mit $x\in \mathbb{C}$ berechnen kann.
...
Der Betrag wird durch das Potenzieren mit $n$ ebenfalls $n$ potenziert, der Polarwinkel jedoch mit $n$ multipliziert.
...
Der Polarwinkel der komplexen Zahl $z = 1+\frac{\theta i}{n}$ extrem nah an $\frac{\theta}{n}$. Wenn man nun $z$ mit $n$ potenziert, dann folgt eine Addition der Winkel. Somit
liegt der Polarwinkel von $z^n$ für große $n$ extrem nah bei $n \cdot \frac{\theta}{n} = \theta$. Dies soll kein mathematischer Beweis sein.
...
Nun lassen sich Überlegungen anstellen, wie sich dieser Punkt auf dem Einheitskreis noch ausdrücken lässt. Hierbei kommen die **trignometrischen Funktionen** ins Spiel.

Die **trignometrischen Funktionen** sind buchstäblich durch den Einheitskreis definiert. Im kartesischen Koordinatensystem lässt sich ein Punkt auf dem Einheitskreis, welcher den Winkel $\theta$ mit der $x$-Achse einschließt mit
...
Dies entspricht der **Eulerschen Formel**, welche sehr nützlich ist um **trignometrische** Zusammenhänge anders auszudrücken.
...
(Mit dem Faktor $\frac{1}{5}$ verlangsamt)
...
<QuestionAnswer>
  <>Wie würde sich der Zeiger $e^{-2 \pi t i}$ verhalten (negatives Vorzeichen im Exponenten)?</>
  <Answer> Die Startposition (bei $t=0$) wäre bei $-1$</Answer>
  <Answer correct={true}>Der Zeiger würde im Uhrzeigersinn rotieren</Answer>
  <Answer>Der Zeiger würde ausschließlich im negativen Bereich rotieren</Answer>
</QuestionAnswer>
...
<QuestionAnswer>
<>Mit welcher komplexen Zahl müsste man $e^{2 \pi  t i}$ multiplizieren um eine Phasenverschiebung von $\frac{\pi}{2}$ hervorzurufen?</>
<Answer>$\frac{\pi}{2} + i$</Answer>
<Answer correct={true}>$i$</Answer>
<Answer>$\frac{\pi}{2}$</Answer>
<Answer>$-1$</Answer>
</QuestionAnswer>
