...
<Callout type="info" emoji="📚">
  ...
  ist. Die Mathematik hinter einer eindeutigen Antwort ist jedoch ziemlich
  kompliziert.
</Callout>
...
$$
c_0 = \frac{1}{3} \qquad \text { und } \qquad  c_m = \frac{2}{m^2\pi^2  }
$$

<FourierSeries
  ithC={(x) => (x == 0 ? [1 / 3, 0] : [2 / Math.pow(Math.PI * x, 2), 0])}
  f={(x) => Math.pow(x - Math.floor(x / 2) * 2 - 1, 2)}
  omega={Math.PI}
  yExtent={[-1, 2]}
/>
