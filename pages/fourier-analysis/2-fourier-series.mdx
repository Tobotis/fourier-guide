...
Also eine Funktion $f(t)$, welche nur für ein bestimmtes Intervall $[a,b]$ betrachetet wird und außerhalb dieses Intervalls $0$ ist.
Zudem scheitert die **Fourier-Reihe**, wenn folgende Funktion analysiert werden soll:

<FrameMafs>
  <Mafs>
    <CartesianCoordinates />
    <FunctionGraph.OfX y={(x) => Math.sin(10 * x) * Math.exp(-1 * x ** 2)} />
  </Mafs>
</FrameMafs>
...
Die Technik der Periodisierung wird im Grunde auch bei der **Fourier-Transfomation** angewandt

<QuestionAnswer>
  <>
    Was könnte man tun um das Signal unabhängig von $a$ und $b$ zu
    periodisieren?
  </>
  <Answer> Eine sehr kleine Periodendauer $T$ wählen. </Answer>
...
Es stellt sich heraus, dass sich damit auch Signale analysieren lassen, welche über keine oszilierenden (schwingenden) Anteile verfügen.
