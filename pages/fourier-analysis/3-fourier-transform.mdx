...
Im Abschnitt über die [Fourier-Reihe](./2-fourier-series), wurde die **Fourier-Reihe** hergeleitet.

Dabei lässt sich zwischen der **Fourier-Reihe** zur **Synthese** einer Funktion $f(t)$, wenn die **Koeffizienten** $c_k$ bekannt sind,
...
Die verschiedenen **Fourier-Koeffizienten** lassen sich darstellen. Im sogenannte **Frequenzraum** werden die **Beträge** der Koeffizienten in Abhängigkeit der zugehörigen Frequenzen aufgetragen.
...
Der Frequenzraum könnte beispielsweise so aussehen:
...
Im **Frequenzraum** lassen sich also die Beiträge der verschiedenen Frequenzen betrachten. Wichtig ist, dass der Abstand auf der horizontalen Achse jeweils $\frac{1}{T}$ entspricht.
Im **Zeitraum**, was der Darstellung des Signals in Abhängigkeit der Zeit bedeutet, folgt:
...
<QuestionAnswer>
  <>
    Was folgt für die Darstellung der Funktion im Frequenzraum, wenn $T$ größer
    wird?
  </>
...
Das Verständis dieses **inversen** Zusammenhangs zwischen den beiden _Räumen_ wird in Bezug auf eine _unendliche_ Periodendauer relevant: Je länger die länger die Periodendauer und somit _breiter_ der Zeitraum, desto _enger_ wird das Frequenzspektrum.
...
Für das Verständis ist es womöglich nützlich nochmal auf die Interpretation dieses Integrals zurückzublicken:
...
Es ergibt sich jedoch ein Problem. Wenn man den Durchschnitt der Funktion $f(t) \cdot e^{\frac{-2 \pi i k}{T} t}$ berechnet, da man mit dem Vorfaktor $\frac{1}{T}$ multipliziert und $T \to \infty$ gelten soll, folgt, dass $c_k \to 0$.
...
$$
|\int_{a}^{b} f(t) e^{\frac{-2 \pi i k}{T}t} dt | < \int_{a}^{b} |f(t)| dt < \infty
$$
...
Es funktioniert also nicht, wenn man einfach $T \to \infty$ und die **Fourier-Reihe** benutzt.

### Herleitung der Fourier Transfomation

Um die obige [Problematik](#problem-mit-den-koeffizienten) zu umgehen, lässt sich der Term

$$
c_k = \frac{1}{T}\int_{-\frac{T}{2}}^{\frac{T}{2}} f(t) e^{\frac{-2 \pi i k}{T} t} dt
$$

mit $T$ skalieren, um somit den Faktor $\frac{1}{T}$ in diesem Term loszuwerden.
Um fortzufahren wird eine neue Notation eingeführt:

$$
\hat{f}(\frac{k}{T}) = \int_{-\frac{T}{2}}^{\frac{T}{2}} f(t) e^{\frac{-2 \pi i  k}{T} t} dt
$$

Es folgt also für die **Fourier-Synthese**:

$$
f(t) = \sum_{k=-\infty}^{\infty} \hat{f}(\frac{k}{T}) e^{\frac{2 \pi i k}{T} t } \frac{1}{T}
$$

Man hat also lediglich den Faktor $\frac{1}{T}$ _verlagert_.

---

**Was passiert nun, wenn man $T$ gegen $\infty$ gehen lässt?**

Relevant ist der Quotient $\frac{k}{T}$, welcher nun aus zwei Variablen besteht, welche gegen unendlich laufen.
...
Wie oben beschrieben wird jedoch der Abstand zwischen zwei verschiedenen ganzzahligen Vielfachen $\frac{k}{T}$ und $\frac{k+1}{T}$ immer kleiner, wenn $T$ größer wird (inverser Zusammenhang zwischen **Frequenzraum** und **Zeitraum**).
Dies ist kein mathematischer Beweise, jedoch lässt sich sagen, dass ein **kontinuirliches** Frequenzspektrum entseht. Sei diese Frequenz $\xi$ (_xi_) (um Verwechlungen mit $f$ und $\hat{f}$ zu vermeiden):
...
Das Integral wird zu einem _uneigentlichen_ Integral. Die Eigenschaft $\int_{-\infty}^{\infty} |f(t)| dt < \infty$ wird also relevant, was jedoch in diesem Artikel jedoch nicht weiter diskutiert wird.
...
Die Unterschiede zwischen zwei Frequenzen $\frac{1}{T}$ werden unendlich klein. Aus diesem Grund wird lässt sich die _diskrete_ Summe, zu einem _kontinuirlichen_ Integral. Die Unterschiede in $\xi$ sind unendlich klein ($d \xi$).
Dieser Zusammenhang wird auch als **inverse Fourier-Transfomation** bezeichnet.
...
Es ist womöglich noch nicht ganz klar, wie sich $\hat{f}(\xi)$ verhält. Der Zusammenhang für die sogenannte **Fourier-Transfomation** von $f(t)$: $\hat{f}(\xi)$, wurde zwar algebraisch aus dem Zusammenhang der **Fourier-Koeffizienten** hergeleitet, durch das Entfernen
des Vorfaktors $\frac{1}{T}$, geht womöglich die Intuition etwas verloren. Die Funktion $\hat{f}(\xi)$ gibt dabei jedoch die Ausoprä

## Beispielanwendung

Im Folgenden soll die **Fourier-Transfomation** angewendet werden:

$$
\hat{f}(\xi) = \int_{-\infty}^{\infty} e^{- 2 \pi i \xi t} f(t)  dt
$$

Dabei soll zu Demonstrationszwecken
