...
Um $\hat{f}(\xi)$ nachzuvollziehen, kann man das Integral genauer betrachten. 
...
Wenn angenommen wird, dass $f(t) \in \mathbb{R}$ und somit ausschließlich **reelle** Funktionswerte annimmt, was bei obiger Funktion der Fall ist, dann gilt Folgendes für den
**Real-** und **Imaginärteil** von $e^{- 2 \pi i \xi t} f(t)$. 

(Folgt mit $e^{xi} = \cos(x) + i \sin(x)$)

<WaveCheck fixed_periodendauer={4} leftBound={-8} rightBound={8}/>
